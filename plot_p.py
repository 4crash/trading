...
class PlotP(object):
    ...
    @staticmethod
    def plot_rsi(  df, ax = "y"):
        """
        docstring
        """
        df.loc[df.kdjk > df.kdjd, "kdjk_color"] = "rgba(26, 150, 65, 0.1)"
        ...
        #         'mode': 'lines', 'line': {'width': 1, 'color': 'green'}, 'name': 'RSI band'}

        # set6 = {'x': [df.iloc[0].index, df.iloc[-1].index], 'y': [30,30],
        #         'type': 'scatter', 'line': {'width': 1, 'color': 'green'}, "fill": 'tonexty', "fillcolor": 'rgba(26, 150, 65, 0.1)'}
    ...
    def plot_bs(ax, df):
        ...
        ax = df.loc[df['sell'] > 0].plot(
            marker='o', y='sell', ax=ax, color='r', linestyle="None", label="sell", alpha = 0.5)
